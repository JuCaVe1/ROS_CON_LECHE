...
//#include <sstream> 	//Añadir librerias o paquetes
 ...
 std_msgs::Int32 Entero_N1;
void Int(const std_msgs::Int32::ConstPtr& Entero_Recibido) //Esta funcion devuelve el mensaje que recibe y es pasado por un impulso shared_ptr esto quiere decir que puede almacenarlo si es necesario
{
	Entero_N1.data=Entero_Recibido->data;
	ROS_INFO("El Entero Recibido Es: [%d]", Entero_Recibido->data); //Es la manera que se usa para imprimir
}
...
{
   ros::init(argc, argv, "Nodo_1_Entero"); //Se llama una de las versiones de ROS, el tercer argumento es el nombre del nodo
	...
	//int16[10] Int;
	ros::Subscriber Sub_entero = n.subscribe("Nodo_1_Entero", 2, Int); 
	ros::Publisher Enviar_Entero_TE = n.advertise<std_msgs::String>("Nodo_1_Enviar_Entero_TE", 2);
 ...
 ros::Rate loop_rate(10); //Permite especificar una frecuencia en la que le gustaría hacer un bucle. En este caso esta funcionando a 10Hz. 
  int count = 0;
 std_msgs::String Entero_N1_TE;
 Entero_N1_TE.data = "";

  while (ros::ok()) 
 {
	if (Entero_N1.data<376)
		Entero_N1_TE.data = "Frio";
	else if (Entero_N1.data<679)
		Entero_N1_TE.data = "Tibio";
	else 
		Entero_N1_TE.data = "Caliente";

	Enviar_Entero_TE.publish(Entero_N1_TE);
